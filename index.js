...
let right_side_bar = document.querySelector(".rightsidebar");
...
function displayData(data) {
  right_side_bar.innerHTML = null;
  data.forEach((el) => {
    let div = document.createElement("div");
    let image_div = document.createElement("div");
    image_div.setAttribute("class", "imageDiv");
    let image = document.createElement("img");
    image.setAttribute("src", `${el.image}`);

    let descriptionDiv = document.createElement("div");
    descriptionDiv.setAttribute("class", "hover-content");
    let descriptionText = document.createElement("div");
    descriptionText.setAttribute("class", "text-content");
    descriptionText.textContent = el.description;

    let brand = document.createElement("p");
    brand.textContent = `Company : ${el.name}`;
    let founder = document.createElement("p");
    let founderShort = shortenString(el.founder, 25)

    founder.textContent = `Founder :  ${founderShort}`;




    let founded = document.createElement("p");
    founded.textContent = `Founded : ${el.founded}`;

    let category = document.createElement("p");
    category.textContent = `Category : ${el.category}`;

    let hq = document.createElement("p");
    hq.textContent = `Hquarter : ${el.headquarter}`;

    descriptionDiv.append(descriptionText);
    image_div.append(image, descriptionDiv);

    let market = document.createElement("p");
    market.textContent = `Market : ${el.market_presence}`;

    let team = document.createElement("p");
    team.textContent = `Team Size : ${el.team_size}`;

    let product_page_button = document.createElement("button");
    product_page_button.setAttribute("class", "product-page");
    product_page_button.textContent = "More";
    
    product_page_button.addEventListener("click",()=>{
   
      localStorage.setItem("product-id",JSON.stringify(el.id));  
      window.location.href="./product.html"
    })
    let visit_button = document.createElement("button");
    visit_button.setAttribute("class", "visit-button");
    visit_button.textContent = "Visit";
    visit_button.addEventListener("click",()=>{
          window.location.href=el.website
    })
    div.append(
      image_div,
      brand,
    ...
    right_side_bar.append(div);
  });
}

displayData(startupData);

let foundingYear = document.querySelector("#foundingYear");
foundingYear.addEventListener("change", (event) => {
